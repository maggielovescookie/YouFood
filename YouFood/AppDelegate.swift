...
class AppDelegate: UIResponder, UIApplicationDelegate, GIDSignInDelegate {
    ...
    func sign(_ signIn: GIDSignIn!, didSignInFor user: GIDGoogleUser!, withError error: Error?) {
        ...
        Auth.auth().signIn(with: credential) { (user, error) in
            if let error = error {
                print("AppDelegate.swift : ERROR - func sign(_ signIn: GIDSignIn!, didSignInFor user: GIDGoogleUser!, withError error: Error?) { -- 1")
                return
            }
            // User is signed in
            self.window?.rootViewController!.performSegue(withIdentifier: "signIn", sender: nil)
        }
    }
    ...
    lazy var persistentContainer: NSPersistentContainer = {
         ...
         */
        let container = NSPersistentContainer(name: "YouFood")
